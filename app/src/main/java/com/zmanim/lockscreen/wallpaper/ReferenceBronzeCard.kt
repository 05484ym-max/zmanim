...
object ReferenceBronzeCard {
    ...
    fun draw(canvas: Canvas, width: Int, height: Int, day: DayZmanim, locationName: String) {
        ...
        // Taller reference-like plaque while staying clear of the Samsung lock-screen clock.
        val cardW = w * .79f
        val cardH = h * .50f
        val top = h * .305f
    ...
    private fun drawPlate(canvas: Canvas, card: RectF) {
        ...
        // Dense deterministic patina: scratches, pores and small warm highlights.
        canvas.save()
        ...
        // Heavy, layered cast-metal frame like the reference.
        val outer = Paint(Paint.ANTI_ALIAS_FLAG).apply {
            style = Paint.Style.STROKE
            strokeWidth = card.width() * .014f
            shader = LinearGradient(
                card.left, card.top, card.right, card.bottom,
                intArrayOf(GOLD_HIGHLIGHT, GOLD, GOLD_DARK, GOLD_LIGHT),
        ...
        canvas.drawRoundRect(
            ...
            Paint(Paint.ANTI_ALIAS_FLAG).apply {
                ...
                color = Color.argb(230, 236, 193, 106)
            }
        )
    }
    ...
    private fun drawHeader(canvas: Canvas, card: RectF, day: DayZmanim, locationName: String) {
        ...
        // Keep room for a reference-style pin at the right edge of the location row.
        ...
        val datePaint = textPaint(w * .034f, IVORY, false).apply {
            ...
            setShadowLayer(w * .004f, 0f, w * .003f, Color.argb(185, 0, 0, 0))
        }
    ...
    private fun drawClock(canvas: Canvas, card: RectF) {
        ...
        val bezel = Paint(Paint.ANTI_ALIAS_FLAG).apply {
            shader = RadialGradient(
                ...
                floatArrayOf(0f, .42f, .74f, 1f),
                Shader.TileMode.CLAMP
            )
        }
        canvas.drawCircle(cx, cy, r * 1.07f, bezel)
        canvas.drawCircle(cx, cy, r * .98f, Paint(Paint.ANTI_ALIAS_FLAG).apply { color = Color.parseColor("#3A2412") })

        val face = Paint(Paint.ANTI_ALIAS_FLAG).apply {
            shader = RadialGradient(
                cx - r * .24f, cy - r * .26f, r,
                intArrayOf(Color.parseColor("#F3D590"), Color.parseColor("#D4A553"), Color.parseColor("#A56D2F")),
                null,
                Shader.TileMode.CLAMP
            )
        }
        ...
        for (i in 0 until 60) {
            ...
            val outer = r * .80f
            val inner = if (i % 5 == 0) r * .63f else r * .72f
        ...
        // Exactly one red seconds hand. No tail and no second red layer.
    ...
    private fun drawGrid(canvas: Canvas, card: RectF, day: DayZmanim) {
        ...
        cells.forEachIndexed { i, cell ->
            ...
            val valuePaint = textPaint(cw * .250f, GOLD_LIGHT, true).apply {
                textAlign = Paint.Align.CENTER
                setShadowLayer(cw * .018f, 0f, cw * .012f, Color.argb(190, 0, 0, 0))
            }
    ...
    private fun drawFooter(canvas: Canvas, card: RectF, day: DayZmanim) {
        ...
        val linePaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
            color = Color.argb(190, 181, 123, 49)
            strokeWidth = w * .0014f
        }
